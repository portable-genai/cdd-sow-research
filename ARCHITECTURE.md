...
| 6 | `ComplianceClientPort` | Regulatory check (`compliance-advisory`) | `platform.remote_compliance` through the portal IAP edge (explicit reuse; `live` reuses it on loopback) | `local.compliance` (in-process) | `platform.remote_compliance` (direct to the sibling) | `onprem.compliance` |
...
| `compliance-advisory` | `compliance-advisory` | `ComplianceClientPort` | `POST /ask` (under `gcp`, through `journey-portal`'s IAP edge at `/apps/compliance-advisory/api`) |
...
| # | Principle (generic) | Mechanism in this repo | Proof |
|---|---------------------|------------------------|-------|
| SC-11 | **WORM plus bounded tamper evidence.** A per-record hash chain detects in-place edits and broken internal links and travels with the export. The chain alone cannot detect tail truncation or a fully recomputed history; that requires an independently protected head anchor or immutable storage. | Locked Cloud Logging bucket (retention a variable, lock a deliberate toggle) in `gcp`; SHA-256 chained SQLite plus optional external head anchor in `local`; both write already-redacted events. A named laptop run sets a damaged, unwitnessed or rolled-back local store aside (renamed, never deleted) and starts a fresh chain rather than refusing. | `cdd-sow audit verify`; the in-place-edit and anchored-history tests in `test_audit_chain.py`; the reset cases in `test_laptop_audit_set_aside.py`. |
| SC-12 | **Record AND detect.** Audit logs nobody reads are not a control: log-based metrics and alert policies surface guardrail blocks, SA-key creation, VPC-SC denials and CMEK changes to an operator. | [`infra/terraform/monitoring.tf`](infra/terraform/monitoring.tf): four security metrics + alert policies behind `posture_alerts_enabled` (channels a variable). | `terraform validate`; with the flag on, alert policies exist even with no channel wired. |
