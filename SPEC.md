# SPEC: `cdd-sow-research` CDD + Source-of-Wealth Agent

The authoritative build specification: locked decisions, the pinned stack, the adapter
convention, the assessment pipeline, and the HTTP contracts `cdd-sow-research` consumes and defines. The
...
`cdd-sow-research` is a grounded-research agent that turns a customer's KYC pack, corporate registries and
...
- Catalog identity: `cdd-sow-research`, group `doc` (document-heavy verticals), priority **P1**, buyer
...
`cdd-sow-research` handles **customer PII** (KYC), so rule **R1** applies: the full `agent-guardrail-gateway` plus DLP
...
- **Governed RAG store is `enterprise-knowledge-base`.** `cdd-sow-research` ingests the case's KYC documents into `enterprise-knowledge-base` (with case ACL
  tags) and retrieves via `enterprise-knowledge-base`. It does not build a separate retrieval backend (that is `enterprise-knowledge-base`'s
  job, rule R3).
...
- Document extraction: **Document AI**. Audit: Cloud Logging locked WORM bucket, retention
  ...
  AI evaluation service plus the `model-quality-gate`.
...
### 6.1 `cdd-sow-research` defines (consumed by the UI/CLI and peers)
...
- `POST /v1/ubo-graph {subject, as_of?}` -> `UboResolution`. Resolves the
  ...
  human review and is routed to `human-review-console` (rule R8). The tenant and the record's ACL are
...
The public UI artifact has one fixed `/agent` base path. Native `journey-portal` hosting keeps
...
one authenticated JSON/form/blob transport. Mode 4 verifies a `cdd-sow-research`-audience OAuth access
token; Mode 5 redeems an iframe-owned PKCE verifier and one-time broker grant for a
dedicated `cdd-sow-research` token. Public citation originals continue through an actor-bound,
...
### 6.3 `cdd-sow-research` consumes (existing platform services)

- **`agent-guardrail-gateway`** (`GUARDRAIL_GATEWAY_URL`): `POST /v1/guardrail/screen {text, direction}`,
  `POST /v1/redact {text}`.
- **`enterprise-knowledge-base`** (`KNOWLEDGE_BASE_URL`): `POST /v1/ingest {document, acl_tags, source_meta}`,
...
`--use-gcp` routes through the Gen AI evaluation service / `model-quality-gate`. Rubrics in `eval/rubrics/`.
...
periodic-review engine, and the same maker-checker route to `human-review-console`.
...
`requires_human_review` and is routed to `human-review-console` through the `ReviewRouterPort`
...
route to `human-review-console`, and it CONVERTS DOWN to `OwnershipSummary`, so the dossier and the
...
to a question the engine answers exactly. The durable record is the `human-review-console` review item plus
