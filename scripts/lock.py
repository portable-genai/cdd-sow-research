...
`tag = commit` map that `tests/unit/test_lockfile_pins.py` checks the pins against.
...
#: (extras, output file) for EVERY lockfile the repo ships, and "every" is load-bearing.
#:
#: `requirements-dev-oidc.lock` was not here, and the only thing that regenerated it was somebody
#: typing the uv command by hand. Nothing tied it to `pyproject.toml`, so it sat pinning
#: superseded releases of agent-eval-kit and hex-service-kit long after both were bumped, and
#: two CI jobs (`oidc-tests` and the browser-evidence job) installed the superseded commons with
#: no diff anywhere to notice. A lockfile outside `make lock` is a lockfile that drifts; the
#: companion `tests/unit/test_lockfile_pins.py` fails the gate if one ever does again.
...
#
# tag -> commit, and `tests/unit/test_lockfile_pins.py` proves each line against the tag
