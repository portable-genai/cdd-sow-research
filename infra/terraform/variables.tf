...
variable "worm_locked" {
  description = <<-EOT
    ...
    #########################################################################
    true is REQUIRED for a compliant production deploy: the audit trail is Write-Once-Read-Many
    only when locked. Set false ONLY for evaluation/demo stacks that must remain deletable
    (terraform destroy works); that posture is NOT compliant and is what this reference
    deployment runs, stated in its own tfvars.

    There is deliberately NO DEFAULT. A plan refuses until the deployment names the lock, because
    an unset value may take a reviewed default and may never take an irreversible one: a `true`
    default once locked a sibling stack's bucket for seven years on a first apply nobody reviewed,
    and the only exit was deleting the project. Fleet-wide as of 2026-09-12; every stack that has
    this control spells it `worm_locked` and none of them defaults it.
  EOT
  type        = bool
}
