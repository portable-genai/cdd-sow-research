...
//
// This console calls its service at `API_BASE` (`/agent/api`, on its own origin), which the Next
// rewrite in `next.config.mjs` proxies to the service with every response header intact, so both
// headers arrive without a route handler having to copy them.
//
...
// to be edited to report what answered it. The transport in `lib/embed/transport.ts` looks
// `fetch` up at call time for exactly this reason. Plain JavaScript so `npm test` runs it in bare
// node.
